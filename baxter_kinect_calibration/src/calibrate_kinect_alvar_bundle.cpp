...
visualization_msgs::Marker makeRvizMarker(string frame, int color, int id, double rad)
{
visualization_msgs::Marker rvizMarker;
...
void resetCorners(MultiMarkerBundle &master, ARCloud &bund_corners)
{
  bund_corners.clear();
...
int correctCorners(MultiMarkerBundle &master, ARCloud &bund_corners, int index, string cloud_frame, string marker_frame)
{
  ...
  for(int j = 0; j < 4; ++j)
  {
      ...
      try{
          tf_listener->waitForTransform(cloud_frame, marker_frame, ros::Time(0), ros::Duration(0.1));
          tf_listener->transformPoint(cloud_frame, p, output_p);      
      }
...
int InferCorners(const ARCloud &cloud, MultiMarkerBundle &master, ARCloud &bund_corners){
  resetCorners(master, bund_corners);  
  ...
  for (size_t i=0; i<kinect_marker_detector.markers->size(); i++)
    {
      ...
      mark_out << "ar_marker_" << id;
      int index = master.get_id_index(id);
      int mast_id = master.master_id;
      
      // But only if we have corresponding points in the pointcloud
      if (index >= 0 && master.marker_status[index] > 0 && marker->valid) {
        n_est++;
        if (correctCorners(master, bund_corners, index, cloud.header.frame_id, mark_out.str()) < 0)
        {
          return -1;
        }
...
int *getTwoValidPoints(const ARCloud &corners_3D, int *dir1, int *dir2)
{
  /*
  int i1,i2;
  if(isCornerNaN(corners_3D[0]) || isnan(corners_3D[3])
  {
    if(isCornerNaN(corners_3D[1]) || isCornerNaN(corners_3D[2]))
    {
      return -1;
    }
    else{
    ...
    int i3,i4;
  if(isCornerNaN(corners_3D[0]) || isCornerNaN(corners_3D[1]))
  {   
    if(isCornerNaN(corners_3D[3]) || isCornerNaN(corners_3D[2]))
    {   
      return -1;
    }
    else{
...
visualization_msgs::Marker makeRvizMarker(string frame, ros::Time stamp, int id, double width, double height, int type)
{
  visualization_msgs::Marker rvizMarker;

  rvizMarker.header.frame_id = frame;
  rvizMarker.header.stamp = stamp;
  rvizMarker.id = id;
  if(type==MAIN_MARKER)
  ...
  rvizMarker.scale.x = width;
  rvizMarker.scale.y = width;
  rvizMarker.scale.z = height;
...
visualization_msgs::Marker makeMarkerMsgs(int type, int id, Pose &p, sensor_msgs::ImageConstPtr image_msg, tf::StampedTransform &CamToOutput){
  
  return makeRvizMarker(image_msg->header.frame_id, image_msg->header.stamp, id, marker_size / 100.0, marker_size / 500.0, type);
}
      ...
      for (size_t i=0; i<marker_detector.markers->size(); i++)
    	{
          ...
          ROS_INFO_STREAM("Pose " << p.translation[0] << ", " << p.translation[1] << ", " << p.translation[2]);
    	    for(int j=0; j<n_bundles; j++){
    	      if(id == master_id[j])
            {
              tf::Transform t = getTransformFromPose(p);
              ROS_INFO_STREAM("Transform: " << t.getOrigin().x() << ", " << t.getOrigin().y() << ", " << t.getOrigin().z());
              broadcastTransform(true, image_msg->header.frame_id, id, t);
            }
    	    }
          
            visualization_msgs::Marker rvizMarker = makeMarkerMsgs(VISIBLE_MARKER, id, p, image_msg, CamToOutput);
            rvizMarkerPub2_.publish (rvizMarker);
    	  }
    	}
      ...
      for(int i=0; i<n_bundles; i++)
    	{
          ...
          tf::Transform markerPose = getTransformFromPose(p);
          ...
          //tf_broadcaster->sendTransform(camToMarker);
          //ROS_INFO_STREAM("Publishing transform from " << image_msg->header.frame_id << " to " << markerFrame);
...
void getPointCloudCallback (const sensor_msgs::PointCloud2ConstPtr &msg)
{
  ...
  if(kinectCam->getCamInfo_)
  {
    try
    {
      ...
      try{
        ros::Time now = ros::Time::now();
        tf_listener->waitForTransform(kinectBaseLinkFrameID, msg->header.frame_id, now, ros::Duration(1.0));
        tf_listener->lookupTransform(kinectBaseLinkFrameID, msg->header.frame_id, now, CamToOutput);
      }
      catch (tf::TransformException ex){
         ROS_ERROR("%s",ex.what());
      }
