...
image_transport::Subscriber calibratedCamSubscriber;
ros::Subscriber kinectCamSubscriber;
...
std::string kinectCamTopic;
std::string kinectCamInfoTopic;
std::string kinectFrame;
std::string calibratedCamTopic;
std::string calibratedCamInfoTopic;
...
void getPointCloudCallback (const sensor_msgs::PointCloud2ConstPtr &msg)
{
  ...
  if(kinectCam->getCamInfo_){
      ...
      for (size_t i=0; i<marker_detector.markers->size(); i++)
    {
      ...
      try{
          tf_listener->waitForTransform("/camera_link", kinectFrame, image_msg->header.stamp, ros::Duration(1.0));
        tf_listener->lookupTransform("/camera_link", kinectFrame, image_msg->header.stamp, camBaseToCamera);
      }
      ...
      std::cout << markerFrame.c_str() << std::endl;
      tf::StampedTransform camToMarker (linkToMarker.inverse(), image_msg->header.stamp, markerFrame.c_str(), "camera_link");
      if (strcmp(markerFrame.c_str(),"ar_marker_5") == 0)
      {
          ROS_INFO_STREAM("Publishing transform between " << markerFrame.c_str() << " and " << "camera_link" << std::endl);
      ...
      try{
        tf_listener->waitForTransform(output_frame, kinectFrame, image_msg->header.stamp, ros::Duration(1.0));
        tf_listener->lookupTransform(output_frame, kinectFrame, image_msg->header.stamp, CamToOutput);
      }
      catch (tf::TransformException ex){
        ROS_ERROR("%s",ex.what());
      }
      //ROS_INFO_STREAM("Transform from frame " << output_frame << " to " << kinectFrame << std::endl << << std::endl);
      ...
      ROS_INFO("Kinect camera info topic does not appear to be correct");
  }
}

void getCapCallback (const sensor_msgs::ImageConstPtr & image_msg)
{
    ROS_INFO("Receiving calibrated image");
    //If we've already gotten the cam info, then go ahead
    if(calibratedCam->getCamInfo_){
        try{
            ...
            marker_detector.Detect(&ipl_image, calibratedCam, true, false, max_new_marker_error, max_track_error, CVSEQ, true);
        ...
        ROS_INFO("Calibrated camera info topic does not appear to be correct");
    }
}
...
int main(int argc, char *argv[])
{
    ...
    if(argc < 10){
        ...
        cout << "Usage: ./individualMarkersNoKinect <marker size in cm> <max new marker error> "
             << "<max track error> <kinect image topic> <kinect info topic> <kinect frame> "
             << "<calibrated image topic> <calibrated info topic> <output frame> [ <max frequency> ]";
        std::cout << std::endl;
        return 0;
    }
    ...
    kinectCamInfoTopic = argv[5];
    kinectFrame = argv[6];
    calibratedCamTopic = argv[7];
    calibratedCamInfoTopic = argv[8];
    output_frame = argv[9];
    marker_detector.SetMarkerSize(marker_size);

    ROS_INFO_STREAM(" Marker Size: " << marker_size << std::endl
                    << " max new marker error: " << max_new_marker_error << std::endl
                    << " max track error: " << max_track_error << std::endl
                    << "kinect image topic: " << kinectImageTopic << std::endl
                    << "kinect camera info topic: " << kinectCamInfoTopic << std::endl
                    << "kinect frame: " << kinectFrame << std::endl
                    << "calibrated camera topic: " << calibratedCamTopic << std::endl
                    << "calibrated camera info topic: " << calibratedCamInfoTopic << std::endl
                    << "output_frame: " << output_frame << std::endl);
    if (argc > 10)
    max_frequency = atof(argv[10]);
  ...
  if (argc > 10)
    ...
    kinectCam = new Camera(n, kinectCamInfoTopic);
    calibratedCam = new Camera(n, calibratedCamInfoTopic);
  ...
  if (enabled == true)
  {
    ...
    ROS_INFO("Subscribing to kinect topic");
    kinectCamSubscriber = n.subscribe(kinectImageTopic, 1, &getPointCloudCallback);
    ROS_INFO("Subscribing to calibrated camera topic");
    calibratedCamSubscriber = it_.subscribe(calibratedCamTopic, 1, &getCapCallback);
  }
  ...
  while (ros::ok())
  {
    ...
    if (enableSwitched == true)
    {
      ...
      if (enabled == false)
      {
        kinectCamSubscriber.shutdown();
        calibratedCamSubscriber.shutdown();
      }
      else
        kinectCamSubscriber = n.subscribe(kinectImageTopic, 1, &getPointCloudCallback);
        calibratedCamSubscriber = it_.subscribe(calibratedCamInfoTopic, 1, &getCapCallback);
